...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, cmd, "version") or std.mem.eql(u8, cmd, "v") or std.mem.eql(u8, cmd, "-v") or std.mem.eql(u8, cmd, "--version")) {
        ...
        //   --bind    tie the session lifetime to its owner clients: when the
        //             last owner (non-mirror) client disconnects, the daemon
        ...
        // zmx claim <session> <cols> <rows>: external focus grant. The pty
        // takes the given size and no client owns it until an owner types.
        const session_name = args.next() orelse return error.SessionNameRequired;
        if (std.mem.eql(u8, session_name, "--help") or std.mem.eql(u8, session_name, "-h")) {
            return help();
        }
        const cols_arg = args.next() orelse return error.SizeRequired;
        const rows_arg = args.next() orelse return error.SizeRequired;
        const size = ipc.Resize{
            .cols = try std.fmt.parseInt(u16, cols_arg, 10),
            .rows = try std.fmt.parseInt(u16, rows_arg, 10),
        };
        if (size.cols == 0 or size.rows == 0) return error.SizeRequired;
        return controlMessage(alloc, &cfg, session_name, .Claim, std.mem.asBytes(&size));
    } else if (std.mem.eql(u8, cmd, "release")) {
        // zmx release <session>: hand the pty size back to the first
        // attached owner client (the PC view).
        const session_name = args.next() orelse return error.SessionNameRequired;
        if (std.mem.eql(u8, session_name, "--help") or std.mem.eql(u8, session_name, "-h")) {
            return help();
        }
        ...
        return stat(alloc, &cfg, session_name);
    } else {
        return help();
    }
}

/// Fire-and-forget control message to a session daemon over a short-lived
/// socket connection (used by `zmx claim`/`zmx release`). Data written to a
/// unix stream socket is delivered even after we close our end.
fn controlMessage(
    ...
    ipc.send(fd, tag, payload) catch |err| switch (err) {
...
fn stat(alloc: std.mem.Allocator, cfg: *Cfg, session_name: []const u8) !void {
    ...
    if (try posix.poll(&poll_fds, 1000) == 0) return error.Timeout;
...
const Client = struct {
    ...
    /// Sent .Init or .InitMirror - a real attached terminal, as opposed to a
    /// short-lived control connection (list/stat/claim/release probes).
    did_init: bool = false,
    /// Mirror clients receive output and may type, but never own the pty
    /// size, never become leader, and never answer terminal queries.
...
const Daemon = struct {
    ...
    // View-bound lifetime (zmx attach --bind): once an owner (non-mirror)
    // client has attached, the daemon shuts down - SIGHUPing the child -
    ...
    pub fn closeClient(self: *Daemon, client: *Client, i: usize, shutdown_on_last: bool) bool {
        ...
        if (self.view_bound and self.has_had_owner and self.countOwners() == 0) {
            std.log.info(
                "view-bound: last owner left, ending session={s}",
                .{self.session_name},
            );
            self.shutdown();
            return true;
    ...
    /// Attached non-mirror clients - the "views" a view-bound session's
    /// lifetime follows.
    fn countOwners(self: *Daemon) usize {
        var owners: usize = 0;
        for (self.clients.items) |q| {
            if (q.did_init and !q.is_mirror) owners += 1;
        }
        return owners;
    }
    ...
    pub fn handleInput(self: *Daemon, client: *Client, payload: []const u8) !void {
        ...
        // Mirrors may type but never claim leadership, and their terminal's
        // auto-replies (cursor/DA reports) are dropped - the leader's
        // terminal already answers the child's queries, and a second reply
        // would corrupt the child's input stream.
        if (client.is_mirror) {
            if (!util.isTerminalReply(payload)) self.queuePtyInput(payload);
            return;
        }
    ...
    pub fn handleInit(
        ...
        if (mirror) {
            // Mirrors get the state snapshot above and the live stream, but
            // never leadership, never resize the pty, and don't count as a
            // "real" terminal client for query-answering purposes.
            client.is_mirror = true;
            return;
        }
        self.has_had_owner = true;
    ...
    pub fn handleResize(
        ...
        if (payload.len != @sizeOf(ipc.Resize)) return;
        // A mirror's local window size is its own rendering concern; it must
        // never resize the pty nor grab a vacant leadership.
        if (client.is_mirror) return;
        if (self.leader_client_fd == null) {
            try self.setLeader(client);
        }
    ...
    pub fn handleDetachAll(self: *Daemon) void {
        ...
        self.clients.clearRetainingCapacity();
        // Detaching every client removes every owner: a view-bound session
        // has no view left to live for.
        if (self.view_bound and self.has_had_owner) self.running = false;
    }

    /// External focus grant (`zmx claim <session> <cols> <rows>`): resize
    /// the pty to the given size and leave leadership vacant. The size
    /// change SIGWINCHes the child, so full-screen TUIs repaint at the new
    /// size. The next owner that types claims leadership (and its size)
    /// back automatically.
    pub fn handleClaim(
        self: *Daemon,
        pty_fd: i32,
        ...
        self.leader_client_fd = null;
        const resize = std.mem.bytesToValue(ipc.Resize, payload);
        if (resize.cols == 0 or resize.rows == 0) return;
        var ws: cross.c.struct_winsize = .{
            .ws_row = resize.rows,
            .ws_col = resize.cols,
            .ws_xpixel = resize.xpixel,
    ...
    /// `zmx release <session>`: hand the pty size back to the first attached
    /// owner client. setLeader asks that client for its window size; the
    /// reply lands in handleResize which resizes the pty.
    pub fn handleRelease(self: *Daemon) !void {
        self.leader_client_fd = null;
        for (self.clients.items) |q| {
            if (q.did_init and !q.is_mirror) {
                std.log.info("release: returning size to fd={d}", .{q.socket_fd});
                try self.setLeader(q);
                return;
            }
        }
...
fn help() !void {
    const help_text =
        ...
        \\
        \\Commands:
        \\  [a]ttach [--bind] [--mirror] <name> [command...]
